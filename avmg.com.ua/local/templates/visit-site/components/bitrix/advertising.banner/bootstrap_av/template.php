...
<div id="carousel-<?=$arResult['ID']?>" class="carousel <?=$arParams['BS_EFFECT']?><?=$arParams['BS_HIDE_FOR_TABLETS']?><?=$arParams['BS_HIDE_FOR_PHONES']?>" data-interval="<?=$arParams['BS_INTERVAL']?>" data-wrap="<?=$arParams['BS_WRAP']?>" data-pause="<?=$arParams['BS_PAUSE']?>" data-keyboard="<?=$arParams['BS_KEYBOARD']?>" data-ride="carousel">
	...
	<?if($arParams['BS_BULLET_NAV'] == 'Y' || $arParams['BS_PREVIEW'] == 'Y'):?>
		...
		<?if(count($arResult['BANNERS']) > 1):?>
			<?while($i < count($arResult['BANNERS'])):?>
				<li data-target="#carousel-<?=$arResult["ID"]?>" data-slide-to="<?=$i?>" <?if($i == 0):?>class="active"<?endif?>></li>
				<?$i++?>
			<?endwhile?>
		<?endif?>
		</ol>
	...
	<!-- Wrapper for slides -->
<div id="av-banner-index">
	<div class="carousel-inner" role="listbox">

	<?foreach($arResult["BANNERS"] as $k => $banner):?>
		<div class="item <?if($k==0) echo 'active';?>">
			<?
				$stringMas = implode(",", [$banner]); //превращаем массив в строку с разделением через запятую
				$cut = substr($stringMas, strpos($stringMas, 'src="') + 5); //обрезаем до первого вхождения
				$ready = mb_strimwidth($cut, 0, strpos($cut, '"')); //обрезаем до второго вхождения
			?>
			<div style="background: url('<?=$ready?>');" class="banner-bg" ><?=$banner?></div>
		</div>
	<?endforeach;?>
